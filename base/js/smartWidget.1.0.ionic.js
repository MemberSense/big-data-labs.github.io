     ...
     init: function(options) {
       ...
       function getScriptDomain() {
        ...
        function loadList($widgetContainer) {
            ...
            */
            ...
            function initiateSlideshow($slideshowContainer) {
                ...
                html[i++] = '<ion-content overflow-scroll="true"><div id="horizontal" class="angularCompile" ng-controller="SlideContainerCtrl">';

                html[i++] = '<i class="fa fa-angle-left fa-4x" ng-click="scrollPrev()"></i>';
                html[i++] = '<i class="fa fa-angle-right fa-4x" ng-click="scrollNext()"></i>';
                html[i++] = '<ion-scroll direction="x" on-drag-up="onDragUp()" on-drag-down="onDragDown()" on-swipe-up="onDragUp()" on-swipe-down="onDragDown()" delegate-handle="isde" class="item wide-item" ng-style="{\'height\': \'600px\'}">';
                html[i++] = '<div style="margin:0px; position:relative;border:1px solid red;width:100%" class="item" collection-repeat="item in getLocations()" collection-item-height="\'100%\'" collection-item-width="\'100%\'" >'
                html[i++] = '<img ng-src="{{item.images[0].large}}" style="width:100%;height:450px;">';
                html[i++] = '<div style="position:absolute;bottom:0px;right:0px;left:0px;color:#fff;background:#000;padding:10px">{{$index}}. {{item.title}}</div>';

                html[i++] = '</div>';

                html[i++] = '</ion-scroll></ion-content>';


                //html[i++] = '<ion-infinite-scroll ng-if="!noMoreItemsAvailable" icon="" on-infinite="loadMore()" distance="0%"></ion-infinite-scroll>';


                html[i++] = '</div>';
                $(".slideContainer").append($(html.join('')));
                compileAngularHtml();
                var slideContainerScope = angular.element(document.getElementById("horizontal")).scope();
                slideContainerScope.items = [];
                ...
                if(imgeAryLn >= 1) {  // used to be imgeAryLn > 1. Allow 1+ images so the kendo slideshow will display.
                    for (var i = 0; i < imgeAryLn; i++) {
                        ...
                        data.push(item);
                        slideContainerScope.$apply(function () {
                           slideContainerScope.items.push(item);
                ...
                //console.log(slideContainerScope.items);
            }
        ...
        function createNavigationMenu($widgetContainer, $listContainer, $slideshowContainer, $galleryContainer, $mapContainer) {
            ...
            html[i++] = '<div class="smartmenuXXX"><ul class="menuList">'; // create the ul element
            html[i++] = liElements; // add the li elements from above.
            html[i++] = '</ul></div>'; // close the ul element
