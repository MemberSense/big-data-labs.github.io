...
var flgFeedLoaded = false, reInitFeed = true, activeLocationMarker = {type:null, marker:null}, globalGmapMapZoomLevel = 8, globalBingMapZoomLevel = 5, GmapPositionMarker = null, BingMapPositionPushpin = null, bingMapPointX = 300, googleMapPanbyX = 250, defaultLocation = {latitude:33.7490, longitude:-84.3892}, flgGmapDragged=false, flgBingDragged=false, changeMarkerColor=true, consoleTime = null, slideshowinterval = null, gmapOverlayCircleAroundMarker = null, gmapOverlayCircle = {
    strokeColor: '#FFFFFF',
    strokeOpacity: 1.0,
    strokeWeight: 2,
    fillColor: '#FFFFFF',
...
(function($) {
    ...
    var defaultOptions = {
        ...
        detailContainer: '#smartDetail',
        navMenuContainer: '#smartNavMenu',
        ...
        displayingItemMap: function() {
        },
        displayedItemMap: function() {
        },
        listItemChecked: function() {
        },
        clickingMenu: function() {  // menuList li
        },
   ...
   var $detailContainer = getWrappedSet(settings.detailContainer);
   var $navMenuContainer = getWrappedSet(settings.navMenuContainer);
   ...
   var methods = {
     ...
     init: function(options) {
       ...
       function getScriptDomain() {
        ...
        function loadList($widgetContainer) {
            ...
            if (settings.createContainers == true) {
                ...
                if ($slideshowContainer == null) {
                    $slideshowContainer = $('<div class="smartSlideshow" data-role="view" data-stretch="true" data-model="viewModel"></div>');
                ...
                if ($detailContainer == null) {
                    $detailContainer = $('<div class="smartDetail"></div>');
            ...
            if ($galleryContainer != null && !$galleryContainer.hasClass('smartGallery')) {
                $galleryContainer.addClass('smartGallery widgets'); // Set display:block, which causes margin to create white space.
                $galleryContainer.addClass('smartGallery');
            }
            if ($mapContainer != null && !$mapContainer.hasClass('mapContainer')) {
                $mapContainer.addClass('smartMap widgets');
            }
            if ($detailContainer != null && !$detailContainer.hasClass('smartDetail')) {
                $detailContainer.addClass('smartDetail widgets');
            }
            ...
            $widgetContainer.on('displayingItemMap.smartWidget', function(event, data) {
                settings.displayingItemMap.call(this, event, data);
            });
            $widgetContainer.on('displayedItemMap.smartWidget', function(event, data) {
                settings.displayedItemMap.call(this, event, data);
            });
            $widgetContainer.on('listItemChecked.smartWidget', function(event, data) {
                settings.listItemChecked.call(this, event, data);
            });
            $widgetContainer.on('clickingMenu.smartWidget', function(event, data) {
                settings.clickingMenu.call(this, event, data);
            });
            $widgetContainer.on('clickedMenu.smartWidget', function(event, data) {
                settings.clickedMenu.call(this, event, data);
            });
            ...
            if (settings.showNavigationMenu == true) {
               ...
               createOptionsDialog($widgetContainer, $listContainer, $slideshowContainer, $galleryContainer, $mapContainer);
            }
            ...
            $.each(settings.feeds, function(feedIndex, feed) {
                ...
                if(typeof($.jsonp) == 'function' && feed.url.indexOf('callback=?') > 0) {
                   ajaxFunction = $.jsonp; // Use the jsonp plugin since jQuery can't detect errors when using jsonp
                }
                ajaxFunction({ url: feed.url, dataType: "json", success: function(feedData, textStatus, jqXHR) {
                    writeConsole('Processing feed ' + (feedIndex+1) + ': <a href="' + feed.url + '">View feed</a>');
                    ...
                    var jsonData = null;
                    console.log(feed.provider.toLowerCase());
                    ...
                    else if (typeof (feed.provider) == 'string' && feed.provider.toLowerCase() == 'allforgood') {
                        console.log("processing allforgood feeds");
                        jsonData = convertallforgoodFeed(feedData);
                    }
                    else {
                        jsonData = feedData;
                    }
                    ...
                    for(var index = startIndex; index < itemArray.length; index++) {
                        ...
                        //if (displayDateString != previousDateString) {
                            if(subHeaderText == '') {
                                ...
                                else if (feed.type == 'good') {
                                    subHeaderText = "Things to Do - All For Good";
                                }
                                else {
                                    subHeaderText = 'Items'; 
                                }
                            ...
                            var checkAllRows = '<div data-feedtype="feedtype_'+feed.type+'" class="listHeaderCheckBoxContainer" style="display: none; float:right; width: 40px;"><input type="checkbox" class="listHeaderCheckBox" id="checkAllCB_' + feed.type + '"/><label for="checkAllCB_' + feed.type + '" style="display: inline;">All</label></div>';
                        ...
                        if(feed.type == 'good') {
                            html[i++] = displayAllForGoodDetails(item);
                        }
                        else {
                            html[i++] = formatContacts(item);
                        }
                        ...
                        rowAttachments = formatAttachments(item);
                        ...
                        venueAddress = formatAddress(item);
                        ...
                        else if (feedType == 'places' || feedType == 'facebookphotos' || feedType == 'facebookevents' || feedType == 'good') {
                        ...
                        html[i++] = formatLineOfBusiness(item);
                        ...
                        html[i++] = formatKeyDetails(item);

                        html[i++] = formatSummary(item, feed.maxSummaryHeadlineLength);
                        ...
                        if ($listContainer != null) {
                            ...
                            if(item.venue != null && "latitude" in item.venue && item.venue.latitude != null && "longitude" in item.venue && item.venue.longitude != null) {
                                ...
                                //html[i++] = "<div style='padding-left:8px' >";
                            ...
                            if(item.venue != null && item.venue.longitude != null && item.venue.latitude != null) {
                                html[i++] = '<a style="font-weight: bold;" href="#" class="gmapDirectionLink" onclick="loadGmapDirection(this, \''+settings.latitude+'\', \''+settings.longitude+'\');return false;" data-location=\'{\"lat\":\"'+item.venue.latitude+'\", \"lang\":\"'+item.venue.longitude+'\"}\' >Directions</a>';
                                
                                maphtml = '<a href="javascript:void(0);" class="rowMapLink" >Map</a>';
                                
                            }

                            // TO DO: Instead place link in static page next to "View QR Code" link when mode=a
                            ...
                            if (!listItemEvent.isDefaultPrevented()) {
                                ...
                                $('.detailsInfo', $rowHtml).hide();
                                if (settings.showListSummary == true) {
                                    //$('.thumbnailContainer > img:first', $rowHtml).css('min-width', '150px'); // Was 120px
                        ...
                        for (image in SmartSlideShowExImgs) {
                            
                            if(mediumImage.indexOf(SmartSlideShowExImgs[image]) > -1 || largeImage.indexOf(SmartSlideShowExImgs[image]) > -1 || thumbnail.indexOf(SmartSlideShowExImgs[image]) > -1 ) {
                    ...
                    if ($slideshowContainer != null) {
                        if(imageArray.length > 0 && settings.showImages) {
                ...
                .fail(function (jqXHR, textStatus, errorThrown) {
                   writeConsole('No data or error for feed ' + (feedIndex+1) + ': <a href="' + feed.url + '">View feed</a>');
            ...
            /*
            if($slideshowContainer != null) {
                settings.showImages ? $slideshowContainer.show() : $slideshowContainer.hide();
            }
            
            if($mapContainer != null) {
                settings.showMap ? $mapContainer.show() : $mapContainer.hide();
            }
            */
            if($listContainer != null) {
                settings.showList ? $listContainer.show() : $listContainer.hide();
            }
            if($galleryContainer != null) {
                settings.showGallery ? $galleryContainer.show() : $galleryContainer.hide();
            }

            // check calendar setting and load it
            if (settings.showCalendar) {
                var cE = document.querySelector('[ng-controller=CalendarCtrl]');
            ...
            function loadMaps() {
                ...
                if (venu && settings.showMap) {
                    //var cE = document.querySelector('[ng-controller=LoadingCtrl]');
                    //var cS = angular.element(cE).scope();
                    //cS.showLoading();
            ...
            $('.googleLink, .googleClusterLink', $widgetContainer).on('click.smartWidget', function(event) {
                ...
                if($(event.currentTarget).hasClass("googleClusterLink")) {
            ...
            $('.esriLink', $widgetContainer).on('click.smartWidget', function(event) {
                window.location = "/esri/map/";
            });
            ...
            */
                    ...
                    if (item.venue) {
                        if (typeof item.venue.latitude != 'undefined' && typeof item.venue.longitude != 'undefined' &&
                            ...
                            google.maps.event.addListener(Static_marker, 'click', function() {
                                ...
                                /*
                                gmapOverlayCircle.map = googlemap;
                                gmapOverlayCircle.center = Static_marker.getPosition();
                                gmapOverlayCircleAroundMarker = new google.maps.Circle(gmapOverlayCircle);
                                */
                                
                            });
                            
                            google.maps.event.addListener(Static_marker, 'mouseover', function() {
                                gmapOverlayCircle.map = googlemap;
                                gmapOverlayCircle.center = Static_marker.getPosition();
                                gmapOverlayCircleAroundMarker = new google.maps.Circle(gmapOverlayCircle);
                            });
                            google.maps.event.addListener(Static_marker, 'mouseout', function() {
                                gmapOverlayCircleAroundMarker.setMap(null);
                            });
                            
                            item.gmarker = Static_marker;
                ...
                google.maps.event.addListener(infowindow, 'domready', function(e) {
                    ...
                    $('.detailsLink', $mapContainer)
                        ...
                        .on('click.smartWidget', function(event) {
                            ...
                            $listRow.find('.detailsLink').click();
                        });
                });
            ...
            function displayMapInfoBox(e) {
                ...
                //if (e.targetType == "pushpin" || e.getLocation()) {
                    ...
                    html[i++] = '<a style="font-weight: bold;" href="javascript:void(0)" onclick="loadGmapDirection(this, \''+settings.latitude+'\', \''+settings.longitude+'\');return false;" class="bingDirectionLink" data-location=\'{\"lat\":\"'+item.venue.latitude+'\", \"lang\":\"'+item.venue.longitude+'\"}\' >Directions</a><br />';
            ...
            if ($listContainer != null) {
                ...
                $listContainer.on("click.smartWidget", function(event, showHideOnly) {
                    ...
                    if($eventTarget.hasClass("listHeaderCheckBox")) {
                        var isChecked = $eventTarget.prop('checked');
                        var feedType = $eventTarget.closest('.listHeaderCheckBoxContainer').data('feedtype');

                        // Get the row checkboxes for this feed.
                        var $selector = $('.eventRow.' + feedType + ' input.rowCheckBox[type="checkbox"]', $listContainer);

                        $selector.each(function(i, element) {
                             var $element = $(element);
                                
                             var $parent = $element.closest('div.eventRow');
                        
                             if ($parent.length == 1) {
                                var dataItem = $parent.data('item');
                                var dataItemFeed = $parent.data('feed');

                                var isRowChecked = $element.prop('checked');

                                // If the row checkbox state differs from the header checkbox state, check or uncheck
                                // the row checkbox to whatever the header checkbox state is. If the checkbox state
                                // already matches the header checkbox state, then there is nothing to do. This prevents
                                // duplicate events from being fired for a row checkbox that already has the correct state.
                                if(isRowChecked != isChecked) {
                    ...
                    if ($eventTarget.hasClass('showAllResults')) {
                        ...
                        if(!allResultsEvent.isDefaultPrevented()) {
                            ...
                            if ($mapContainer != null && typeof(panorama) != 'undefined') {
                                $(".gpsMapButton", $mapContainer).show();
                                panorama.setVisible(false);
                            }
                    ...
                    */
                    ...
                    /*
                     * When clicking a title (eventRowInfo) in the itemList, the text below it is disappearing.
 
                    1. Remove this click event from the eventRowInfo and the rollover css now that the "Detail" link is prominently displayed in the item row.
                    http://georgiafacts.org/base/nearby/

                    Side note: In the future we might have a request to add an "expand" area on each item row in the list.
                    ...
                    if ($link.length == 1) {
                        ...
                        if ($parent.length == 1) {
                            ...
                            if(typeof(showHideOnly) == 'undefined' || typeof(showHideOnly) == 'boolean' && showHideOnly == false) {
                                if(dataItem.venue != null && dataItem.venue.latitude != null && dataItem.venue.longitude != null) {
                            ...
                            if ($link.hasClass('detailsLink')) {
                                ...
                                if($link.text() == 'Details') { // BUGBUG - remove this so we can change the link title.
                                    ...
                                    if(!detailsEvent.isDefaultPrevented()) {
                                        ...
                                        if($slideshowContainer != null) {
                                            $slideshowContainer.find('.pause').click();
                                        }
                                        ...
                                        getDetails($detailsInfo, dataItem, dataItemFeed, function() {
                                            ...
                                            if($detailsInfo.is(':empty')) {
                                                ...
                                                if($detailContainer != null) {
                                                    var $fragment = $('<div class="closeDetail closeIcon entypo-cancel-squared" style="float:right;">')
                                                        .click(function(event) {
                                                           $detailContainer.hide();
                                                        });
                                                    $detailContainer.empty().append($fragment);
                                                    $detailsInfo.clone().show().appendTo($detailContainer);
                                            ...
                                            if($slideshowContainer != null) {
                                                ...
                                                if(imageIndex >= 0) {
                                                    //pauseShow();
                                                    //goIndex(imageIndex);
                                                    $(".slideContainer").data("kendoMobileScrollView").scrollTo(imageIndex);
                                                }
                                            }
                               ...
                               console.log('summaryLink');
                                            ...
                                            if(dataItemFeed.detailsUrl != '' && dataItemFeed.detailsUrl != null
                                             && (typeof(showHideOnly) == 'undefined' || (typeof(showHideOnly) == 'boolean' && showHideOnly == false))) {
                                                ...
                                                getDetails($detailsInfo, dataItem, dataItemFeed, function() {
                                                    ...
                                                    if(!$detailsInfo.is(':empty')) {
                                                        if($detailContainer != null) {
                                                            var $fragment = $('<div class="closeDetail closeIcon entypo-cancel-squared" style="float:right;"></div>')
                                                                .click(function(event) {
                                                                   $detailContainer.hide();
                                                                });
                                                            $detailContainer.empty().append($fragment);
                                ...
                                // displayingItemMap Event
                                var itemMapEvent = jQuery.Event('displayingItemMap');
                                var eventData = {
                                    item: dataItem,
                                    feed: dataItemFeed
                                };
                                
                                $parent.trigger(itemMapEvent, eventData);
                                if(!itemMapEvent.isDefaultPrevented()) {
                                    ...
                                    if (previous_highlighted_marker != null) {
                                        previous_highlighted_marker.setIcon(previous_highlighted_marker.picon);
                                    }
                                    if(dataItem.venue != null && dataItem.venue.longitude != null && dataItem.venue.latitude != null && typeof google != "undefined") {
                                        ...
                                        previous_highlighted_marker.picon = dataItem.gmarker.getIcon();
                                    ...
                                    // displayedItemMap Event
                                    itemMapEvent = jQuery.Event('displayedItemMap');
                                    $parent.trigger(itemMapEvent, eventData);
            ...
            if ($slideshowContainer != null) {
                ...
                if($galleryContainer != null) {
                    $galleryContainer.on('click.smartWidget', function(event) {
                        ...
                        if ($imageContainer.length == 1) {
                            ...
                            $(".slideshowContainer").show();
                            $(".slideContainer").data("kendoMobileScrollView").scrollTo(imageIndex);
                            //$slideshowContainer.show(); // Don, this didn't work.
                            //$widgetContainer.find('#smartSlideshow').show(); // Don, this didn't work.
                ...
                /*
                 * 
                 * auto play slide show functionality removed for now.
                ...
                */
                ...
                function playSlideShow() {
                    $slideshowContainer.find('.next').click();
                    console.log("next clicked");
                }
                $slideshowContainer.find('.pause').on('click.smartWidget', function(e) {
                    $slideshowContainer.find('.pause').hide().next('.resume').show();
                    clearInterval(slideshowinterval);
                    //console.log(e.data.callback);
                    console.log("paused clicked");
                });
                $slideshowContainer.find('.resume').on('click.smartWidget', function(e) {
                    $slideshowContainer.find('.resume').hide().prev('.pause').show();
                    if($detailContainer != null) {
                        $detailContainer.find('.closeDetail').click();
                    }
                    slideshowinterval = setInterval(playSlideShow, 5000);
                    //console.log(e.data.callback);
                ...
                $slideshowContainer.find('.prev').on('click.smartWidget', function() {
                    ...
                    var $kendoMobileScrollView = $(".slideContainer").data("kendoMobileScrollView");
                    if(typeof($kendoMobileScrollView) != 'undefined') {
                        if ($(".km-pages li:first").hasClass("km-current-page")) {
                            $kendoMobileScrollView.scrollTo($(".km-pages li").length - 1, true);
                        } else {
                            $kendoMobileScrollView.prev();
                        }
                    }
                });
                ...
                $slideshowContainer.find('.next').on('click.smartWidget', function() {
                    ...
                    var $kendoMobileScrollView = $(".slideContainer").data("kendoMobileScrollView");
                    if(typeof $kendoMobileScrollView != 'undefined') {
                        if ($(".km-pages li:last").hasClass("km-current-page")) {
                            $kendoMobileScrollView.scrollTo(0, true);
                        } else {
                            $kendoMobileScrollView.next();
                        }
                    }
                });
            ...
            if (settings.showNavigationMenu == true) {
                var $menuContainer = $widgetContainer;
                if($navMenuContainer != null) {
                   $menuContainer = $navMenuContainer;
                }
                $menuContainer.find('.menuIcon').on('mouseenter.smartWidget', function() {
                    //$widgetContainer.find('.menuList').fadeIn(); // Was this needed for iPad? Added following instead:
                    $menuContainer.find('.menuList').show();
                    return false;
                });
                $menuContainer.find('.menuList').on('mouseleave.smartWidget', function() { //the handlerOut
                    //$widgetContainer.find('.menuList').fadeOut(); // Was this needed for iPad? Added following instead:
                    if (settings.showMenuIcon == true) {
                        $menuContainer.find('.menuList').hide();
                    }
                    return false;
                });

                $('.menuList li').on('click.smartWidget', function(event) {
					var divcls = $(this).attr('divid');
                    
                    // clickingMenu event
                    var clickMenuEvent = jQuery.Event('clickingMenu');
                    var eventData = {
                        id: divcls
                    };

                    $widgetContainer.trigger(clickMenuEvent, eventData);
                    if(!clickMenuEvent.isDefaultPrevented()) {

                        //$('.widgets').hide(); // This caused smartList to be hidden when choosing newSearch - which is good since it scrolls with search.
                        if (divcls == 'galleryContainer') {
                            ...
                            $('#smartGallery').show();
                            $('#slideshowContainer').show();
                            $('html,body').animate({ scrollTop: $('#smartSlideshow').offset().top }, 'slow');
                            $('.basicSearch').hide();
                            $('.galleryContainer').hide();
                        } else if (divcls == 'smartMap') {
                            $('.widgets').hide();
                            $('#smartMap', $widgetContainer).show();
                        } else if (divcls == 'widgets') {
                            $('.' + divcls).show();
                            ...
                            $("#smartList").addClass("widgetBesideMenu");
                            $("#smartMap").addClass("widgetFixedBesideMenu");
                            //alert("here");
                            $('#smartMap').show();
                            $('.basicSearch').show();
                        } else if (divcls == 'smartList') {
                            $('#smartMap').show();
                            $('.' + divcls).show();
                            $('html,body').animate({ scrollTop: $('#smartList').offset().top }, 'slow');
                        }/*
                        else { // smartList
                            //$('.basicSearch').hide();
                            $('#smartMap').show();
                        ...
                        */
                        ...
                        // clickedMenu event
                        clickMenuEvent = jQuery.Event('clickedMenu');
                        $widgetContainer.trigger(clickMenuEvent, eventData);
                    }
                });
            }
                ...
                if (!ignoreHashChange) {
                    ...
                    $.each(imageArray, function(index, item) {
                        ...
                        if (itemArray[itemIndex].id == slideID) {
                            //goIndex(index);
                            $(".slideContainer").data("kendoMobileScrollView").scrollTo(index);
            ...
            function initiateSlideshow($slideshowContainer) {
                ...
                imgeAryLn = imageArray.length;
                if(imgeAryLn >= 1) {  // used to be imgeAryLn > 1. Allow 1+ images so the kendo slideshow will display.
                    for (var i = 0; i < imgeAryLn; i++) {
                        var item = itemArray[imageArray[i]];
                        //console.log(item);
                        var itemFeed = settings.feeds[item.feedIndex];
                        item.host = host;
                        ...
                        if (itemFeed.type == 'locations' || itemFeed.type == 'places') {
                            ...
                            /*
                            htmlSpan[j++] = formatLineOfBusiness(item);
                            htmlSpan[j++] = formatAddress(item);
                            htmlSpan[j++] = formatKeyDetails(item);
                            htmlSpan[j++] = formatSummary(item);
                            htmlSpan[j++] = formatContacts(item);
                            htmlSpan[j++] = formatAttachments(item);
                            */
                            ...
                            item.slide_summary = htmlSpan.join('');
                        ...
                        item.itemFeed = {detailsUrl:itemFeed.detailsUrl};
                ...
                //html[i++] = '#if(host + images[0].large != "https://m.ak.fbcdn.net/profile.ak/hprofile-ak-xaf1/t1.0-1/c59.0.200.200/p200x200/418333_10149999285994467_1920585607_n.png") {#'
                ...
                html[i++] = '#if(title) {# <div class="slide-title">#: title #</div> #}#';
                
                html[i++] = '#if(typeof(slide_summary) != "undefined") {# <span class="slide-summary">#: slide_summary # </span> #}#';
                
                html[i++] = '<div class="fullDetailLinks" style="margin-left:0px">'; // SlideShow
                html[i++] = '#if(venue != null && venue.longitude != null && venue.latitude != null) {# <a style="font-weight: bold;" href="javascript(void);" class="gmapDirectionLink" onclick="loadGmapDirection(this, \'#: settings.latitude #\', \'#: settings.longitude #\');return false;" data-location=\'{"lat":"#: venue.latitude #", "lang":"#: venue.longitude #"}\' >Directions</a>#}#';
                //html[i++] = '<a href="#:htmlLink #" data-id="#: id #" target="_blank">Page</a>';
                html[i++] = '#if(itemFeed.detailsUrl != "" && itemFeed.detailsUrl != null) {#<a href="" data-id="#: id #" class="detailsLink">Details</a> #}#';
                html[i++] = '</div>';
                ...
                html[i++] = '<div class="slide-class active" style="background:url(#: host + images[0].large #);background-size:cover;">';
                html[i++] = '<img style="display:none;" class="slide-image lazyLarge grab-cursor" src="#: host + images[0].large #" alt="">';
                html[i++] = '</div>';
                //html[i++] = '#}#'
                
                
                $(".slideContainer").kendoMobileScrollView({
                    dataSource: new kendo.data.DataSource({
                                    data: data
                                }),
                    enablePager: imgeAryLn <= 12,
                    //enablePager: true,
                    contentHeight: "100%",
                    duration: 1000,
                    //template: kendo.template($("#tmpl").html())
                    template: kendo.template(html.join(''))
                });
                slideshowinterval = setInterval(playSlideShow, 5000);
                /*
                 * 
                 * The slide-text does not appear on the first slide, but if I advance forward two slides and then back up, it appears.
                 * Kendo is adding display:none in slide-text
                $(".slide-text, slide-class").kendoMobileView({
                    zoom: true
                });*/
                
                
                
                $("ol.km-pages li").click(function(e) {
                    $(".slideContainer").data("kendoMobileScrollView").scrollTo($(this).index());
                });
                /*
                $(".km-virtual-page").kendoTouch({
                    filter: ".active",
                    enableSwipe: true,
                    minXDelta: 50,
                    swipe: function(e) {
                ...
                // AngularJS Touch Carousel Starts here
                carouselData = data;
                $("#smartAngularCarouselHolder").html("<div style=\"height:600px;\" class=\"angularCompile slideshowContainer\" id=\"carousel-ctrl\" ng-controller=\"AngularCarouselCtrl\"></div>");
                compileAngularHtml();
                var cE = document.querySelector('[ng-controller=AngularCarouselCtrl]');
                console.log("angular" + cE);
                var cS = angular.element(cE).scope();
                cS.loadngcarousel();
                
                // AngularJS Touch Carousel Ends here
        ...
        function createSlideshowControls($slideshowContainer) {
            ...
            var i = 0;
            var html = [];
            html[i++] = '<div class="slideshowContainer" id="slideshowContainer">'; // same as slideshow-holder in plugin.html
        ...
        function createNavigationMenu($widgetContainer, $listContainer, $slideshowContainer, $galleryContainer, $mapContainer) {
            ...
            html[i++] = '<li divid="newSearch"><span>New Search</span></li>';
            if ($slideshowContainer != null) {
                html[i++] = '<li class="toggleSlides" divid="slideshowContainer"><span>Slideshow</span></li>';
            }
            if ($galleryContainer != null) {
                html[i++] = '<li class="toggleGallery" divid="galleryContainer"><span>Gallery</span></li>';
            }
            if ($galleryContainer != null) {
                //html[i++] = '<li class="toggleCalendar" divid="smartCalendarHolder"><span>Calendar</span></li>';
            }
            html[i++] = '<li><div class="angularCompile" ng-controller="CalendarCtrl" ng-click="loadCalendar()">Calendar</div></li>';
            //html[i++] = '<li><div class="angularCompile" ng-controller="CalendarCtrl" ng-click="addEvent()">Load Calendar</div></li>';
            if ($mapContainer != null) {
                //html[i++] = '<li class="toggleMap" divid="smartMap">Map</li>';
                html[i++] = '<li class="toggleMap" divid="smartMap"><span>Map</span>';
               
                /*
                html[i++] = '<div class="angularCompile" ng-controller="mapOptionCtrl">';
                html[i++] = '<ion-radio ng-repeat="item in mapOptionsList"';
                html[i++] = 'ng-value="item.value"';
                html[i++] = 'ng-model="selection.optionValue"';
                html[i++] = 'name="mapOptions"';
                html[i++] = 'ng-class="item.class"';
                html[i++] = 'ng-style="item.style"';
                html[i++] = 'ng-change="selectionChanged(item)"';
                html[i++] = ' />';
                html[i++] = '{{ item.text }}';
                html[i++] = '</ion-radio>';
                //html[i++] = '<div ng-click="loadCalendar()">Load Calendar</div>'; //TODO - Move into a different controller
                html[i++] = '</div>';
                */
                
                // Later we'll make these a popup by the smartMap.
                //html[i++] = '<ul>';
                //html[i++] = '<li><a href=""><span>Google</span></a></li>';
                //html[i++] = '<li><a href=""><span>Google Cluster</span></a></li>';
                //html[i++] = '<li styleXX="display:none" class="user-9"><a href=""><span>JPEG Map</span></a></li>';
                //html[i++] = '<li class="last"><span><ion-radio ng-model="choice" ng-value="A"> &nbsp; &nbsp;Bing Map</ion-radio></span></li>';
                //html[i++] = '<li styleXX="display:none" class="user-9"><a href="/esri/map/"><span>ESRI Map</span></a></li>';
                //html[i++] = '</ul>';

                html[i++] = '</li>';
            }
            if ($listContainer != null) {
                html[i++] = '<li class="toggleListings" divid="smartList"><span>Listings</span></li>';
            }
            html[i++] = '<li class="toggleAll" divid="widgets"><span>Show All</span></li>';
            html[i++] = '<li class="pdfLink" divid="widgets"><span><div class="fa fa-file-pdf-o"></div> PDF</span></li>';
            if (i <= 1) {
            ...
            html[i++] = '</div>'; // close smartNavMenu

            if($navMenuContainer == null) {
                $widgetContainer.append($(html.join('')));
            }
            else {
                $navMenuContainer.append($(html.join('')));
            }
            compileAngularHtml();
        }

        function createOptionsDialog($widgetContainer, $listContainer, $slideshowContainer, $galleryContainer, $mapContainer) {

            return; // skip this for now - may need later.

            var i = 0;
            var html = [];

            html[i++] = '<div class="angularCompile" ng-controller="SettingsDialog">';
            html[i++] = '<script id="my-modal.html" type="text/ng-template">';
            html[i++] = '<ion-modal-view id="optionsDlg" style="z-index: 100;">';
            html[i++] = '<ion-header-bar>';
            html[i++] = '<h1 class="title">My Modal title</h1>';
            html[i++] = '</ion-header-bar>';
            html[i++] = '<ion-content>';
            html[i++] = 'Hello!';
            html[i++] = '</ion-content>';
            html[i++] = '</ion-modal-view>';
            html[i++] = '</script>';
            html[i++] = '</div>';
        ...
        function  prepareFullCalendar() {
            //lazy load the calendar js
            async_load("../bower_components/jquery-ui/ui/jquery-ui.js")
            async_load("../bower_components/moment/moment.js")
            async_load("../bower_components/angular/angular.js")
            async_load("../bower_components/angular-ui-calendar/src/calendar.js")
            async_load("../bower_components/fullcalendar/dist/fullcalendar.js")
            async_load("../bower_components/fullcalendar/dist/gcal.js")
            
        }*/
        function convertallforgoodFeed(jsonData) {
            ...
            compileAngularHtml();*/
            /* add event to angular-ui calendar */
            ...
            $.each(jsonData.items, function(i, jsonItem) {
                ...
                */
                ...
                item.htmlLink = jsonItem.detailUrl;
                item.guid = jsonItem.detailUrl;
                item.created = jsonItem.pubDate;
                item.updated = jsonItem.pubDate;

                var creator = {};
                item.creator = creator;
                creator.displayName = jsonItem.sponsoringOrganizationName;
                creator.firstName = null;
                creator.lastName = null;
                creator.email = null;
                creator.phone = null;
                ...
                var contact = null;
                if(jsonItem.contactName != '' || jsonItem.contactPhone != '' || jsonItem.contactEmail != '') {
                    item.contacts = [];
                    contact = {};
                    item.contacts.push(contact);
                    contact.displayName = jsonItem.contactName;
                    contact.firstName = null;
                    contact.lastName = null;
                    contact.email = jsonItem.contactEmail;
                    contact.phone = formatPhone(jsonItem.contactPhone);
                }

                // "2014-09-07 00:00:00" => "2014-09-07T00:00:00"
                item.start = new Date(jsonItem.startDate.replace(' ', 'T'));
                item.end = new Date(jsonItem.endDate.replace(' ', 'T'));
                    
                
                item.location = jsonItem.location;
                var venue = {};
                venue.street = jsonItem.street1;
                venue.city = jsonItem.city;
                venue.state = jsonItem.region;
                venue.zip = jsonItem.postalCode;
                venue.country = jsonItem.country;
                var latlong = jsonItem.latlong.split(",");
                venue.latitude = latlong[0];
                venue.longitude = latlong[1];
                venue.addressString = jsonItem.location_name.trim(); // full address including street. The jsonItem.street1 field in the feed is empty.  
                item.venue = venue;

                var organizer = {};
                item.organizer = organizer;
                organizer.displayName = jsonItem.sponsoringOrganizationName;
                organizer.firstName = null;
                organizer.lastName = null;
        ...
        function displayAllForGoodDetails(item) {
            var i = 0;
            var html = [];

            var dateString = '';
            var startDate = days[item.start.getDay()] + ", " + months[item.start.getMonth()] + ' ' + item.start.getDate() + ', ' + fourdigits(item.start.getYear());
            var endDate = days[item.end.getDay()] + ", " + months[item.end.getMonth()] + ' ' + item.end.getDate() + ', ' + fourdigits(item.end.getYear());

            if(item.start.toDateString() != item.end.toDateString()) {
                // multiple days
            ...
            var startHoursString = item.start.getHours().toString() + item.start.getMinutes().toString();
            var endHoursString = item.end.getHours().toString() + item.end.getMinutes().toString();
            
            if(startHoursString != '00' && endHoursString != '2359' && endHoursString != '00') {
                var startTimeParts = item.start.toLocaleTimeString().split(' ');
                if(startHoursString != endHoursString) {
                    var endTimeParts = item.end.toLocaleTimeString().split(' ');

                    // Trim off the milliseconds from the time string
                    dateString += ' ' + startTimeParts[0].slice(0, startTimeParts[0].lastIndexOf(':')) + ' ' + startTimeParts[1]  + ' - ' + endTimeParts[0].slice(0, endTimeParts[0].lastIndexOf(':')) + ' ' + endTimeParts[1];
                }
                else {
                    // Start/End Times match - just display the start time.
                    // Trim off the milliseconds from the time string
                    dateString += ' ' + startTimeParts[0].slice(0, startTimeParts[0].lastIndexOf(':')) + ' ' + startTimeParts[1];
                }
            }


            html[i++] = '<b>Date:</b> ' + dateString + '<br />';
            if(item.organizer != null && item.organizer.displayName != null && item.organizer.displayName != '') {
                html[i++] = '<b>Sponsor:</b> ' + item.organizer.displayName + '<br />';
            }
            if(item.venue != null && item.venue.addressString != null && item.venue.addressString != '') {
                html[i++] = '<b>Where:</b> ' + item.venue.addressString + '<br />';
            }

            if(item.contacts != null) {
                $.each(item.contacts, function(index, contact) {
                    var contactString = '<b>Contact:</b> ';
                    if(contact.displayName != null && contact.displayName != '') {
                       contactString += contact.displayName + ', ';
                    }
                    if(contact.phone != null && contact.phone != '') {
                       contactString += contact.phone + ', ';
                    }
                    if(contact.email != null && contact.email != '') {
                       contactString += contact.email + ', ';
                    }
                    contactString = contactString.substr(0, contactString.length - 2); // remove trailing comma and space
                    html[i++] = contactString + '<br />';
                    return false; // should be only one contact
                });
            }

            html[i++] = '<b><a href="' + item.htmlLink + '" target="volunteerWindow">Details</a></b><br />';
        ...
        function formatContacts(item) {
            ...
            if(item.contacts != null) {
                html[i++] = '<div style="margin-bottom:8px">';
                html[i++] = '<b style="font-size:.9em">Contacts</b><br>';
                $.each(item.contacts, function(index, contact) {
        ...
        function formatAttachments(item) {
            var i = 0;
            var html = [];
        ...
        function formatLineOfBusiness(item) {
            var i = 0;
            var html = [];
        ...
        function formatKeyDetails(item) {
            var i = 0;
            var html = [];
        ...
        function formatSummary(item, maxLength) {
            var i = 0;
            var html = [];
            ...
            if (item.summary != null && item.summary != '') {
                if(!isNaN(maxLength)) {
                    html[i++] = '<div>';
                    html[i++] = '<span class="summaryHeadline">' + getSubstringWords(item.summary, maxLength) + '</span>';
                    html[i++] = '</div>';
                }
                else {
                    html[i++] = item.summary;
        ...
        function formatPhone(phone) {
            ...
            // If the phone number has letters, don't try to format it
            var regex = /[A-Za-z]/;
            if(regex.test(phone)) {
               return phone;
            }

            // Remove any characters not matching a digit
            regex = /([^0-9])/g;
            var phoneNoFormat = phone.replace(regex, '');
            var phoneFormatted = '';

            if (phoneNoFormat.length >= 10)
            {
                if(phoneNoFormat.length == 10) {
                    // number with area code
                    phoneFormatted = phoneNoFormat.substr(0, 3) + '-' + phoneNoFormat.substr(3, 3) + '-' + phoneNoFormat.substr(6);
                }
                else if(phoneNoFormat.length == 11) {
                    // U.S. long distance number
                    phoneFormatted = phoneNoFormat.substr(0, 1) + '-' + phoneNoFormat.substr(1, 3) + '-' + phoneNoFormat.substr(4, 3) + '-' + phoneNoFormat.substr(7);
                }
                else {
                    // some other international number
            ...
            return phone; // too short, just return the phone number as is
        }

        function compileAngularHtml() {
            ...
            $(".angularCompile").each(function () {
                var $content = $(this);
                $content.removeClass('angularCompile'); // We only need to compile once
                
                // Get the $compile module so we can compile the html with the angular directives.
                angular.element(document).injector().invoke(function($compile) {

                    // Get/Create the scope for the element
                    var scope = angular.element($content).scope(); 
                    
                    // Compile the html into a template. It returns a function that can be used to 
                    // bind/link the template to the scope
                    var fnLink = $compile($content);  
                    
                    fnLink(scope); // Bind the scope to the template
                    
                    scope.$apply(); // Process the template using the scope
                });
            });
        }
    ...
    checkBoxes: function(action, feedType) {
       this.each(function() {
           ...
           var $selector;
           if(feedType != '') {
              $selector = $('.feedtype_' + feedType + ' .rowCheckBoxContainer' + ', .listHeaderCheckBoxContainer[data-feedtype="feedtype_' + feedType + '"]', $this);
           }
           else {
              $selector = $('.rowCheckBoxContainer, .listHeaderCheckBoxContainer', $this);
           }
    ...
    function getWrappedSet(selector) {
        ...
        if ($wrappedSet != null && $wrappedSet.length == 0) {
            $wrappedSet = null;
        }
        return $wrappedSet;
    }
...
function addhover(settings) {
    if (settings.showMenuIcon == true) {
        ...
        if ('ontouchstart' in document.documentElement) {
            ...
            // BUGBUG - adding padding or BRs within top of smartWidget div prevents mouseenter from reaching menIcon.
            $('#smartWidget div')
            .on('mouseenter.smartWidget', function(e) {
                $('.menuIcon').show();
            })
            .on('mouseleave.smartWidget', function() {
                // BUGBUG - Reactivate hiding once mouseenter reaches menuIcon.
                //$('.menuIcon').hide();
            });
        }
    }
...
$.cookie.json = true;
$.cookie.defaults.expires = 1; // expires after 1 day
...
function zoomInMap()
{
    ...
    if ($('.bingMapsHolder').is(":visible")) {
        ...
        if (typeof(panorama) != 'undefined' && panorama.getVisible()) {
...
/*   Dropdown plugin code starts     */

(function ($) {
    
    $.fn.divBasedSelectBox = function (options) {
        ...
        return this.each(function () {
            ...
            /* Create HTML */
            if (typeof obj.options === 'object' && Object.keys(obj.options).length > 0) {
                
                    if (typeof obj.selected !== 'undefined' && obj.selected !== '' && obj.options.hasOwnProperty(obj.selected)) {
                        $this.data("text", obj.options[obj.selected]);
                        $this.data("value", obj.selected);
                        
                        $html.push('<div class="filter">');
                        
                        $html.push('<span class="filterSelected" data-selected="'+obj.selected+'">'+obj.options[obj.selected]+'</span>');
                        $html.push('<i class="entypo-down-open" style="font-size:13pt"></i>');
                        $html.push('</div>');
                    }
                
                
                $html.push('<div class="filterBubbleHolder" style="display:none;">');
                $html.push('<div class="uparrow uparrow-grey" style="left: 100px;"></div>');
                $html.push('<div class="uparrow uparrow-white" style="left: 100px;"></div>');
                $html.push('<div class="filterBubble" style="min-width:180px">');
                $html.push('<ul class="filterUL">');
                
                $.each(obj.options, function(key, value) {

                    $html.push('<li data-id="'+key+'" data-value="'+key+'"><a style="white-space:nowrap" href="#">'+value+'</a></li>');
            ...
            $this.find(".filter").on("click", function(e) {
                $this.find(".filterBubbleHolder").toggle();
            });
            
            $this.find("li").on("click", function(e) {
                
                var $thisli = $(this),
                    $filterSelected = $this.find(".filterSelected"),
                    $thislivalue = $thisli.data("value");
                
                $filterSelected.text($thisli.text());
                $filterSelected.data('value', $thislivalue);
                $filterSelected.data('selected', $thislivalue);
                
                $this.data("text", $thisli.text());
                $this.data("value", $thisli.data("value"));
